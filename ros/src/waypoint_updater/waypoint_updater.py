...
class WaypointUpdater(object):
    ...
    def loop(self):
		if(self.waypoint and self.currnet):
			...
			if self.traffic_waypoint and self.traffic_waypoint != -1:
				tl_dist = self.distance(self.waypoint.waypoints, index, self.traffic_waypoint)
	...
	def getWaypoints(self, waypoints):
        ...
        for wp in waypoints[:-1][::-1]:
            dist = self.disttemp(wp.pose.pose.position, last.pose.pose.position)
